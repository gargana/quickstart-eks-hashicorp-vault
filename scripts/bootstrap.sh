...
while true; do
  IP_ADDRESS=$(kubectl get pods vault-0 -o jsonpath={.status.podIP})
  if [ "${IP_ADDRESS}X" == "X" ]
  then
    sleep 1
  else
...
if [ "$init" != "Vault is initialized" ]; then
    ...
    SECRET_VALUE=$(kubectl exec vault-0 -- vault operator init -recovery-shares=${VAULT_NUMBER_OF_KEYS} -recovery-threshold=${VAULT_NUMBER_OF_KEYS_FOR_UNSEAL})
...
kubectl exec vault-0 -- vault login token=$root_token 2>&1 > /dev/null  # Hide this output from the console

# Join other pods to the raft cluster
kubectl exec -t vault-1 -- vault operator raft join http://vault-0.vault-internal:8200

kubectl exec -t vault-2 -- vault operator raft join http://vault-0.vault-internal:8200

# Show who we have joined
kubectl exec -t vault-0 -- vault operator raft list-peers
