...
(`VaultDeploymentSize`)|`small`|Deployment size of dedicated HashiCorp Vault nodes.|Kubernetes namespace for Vault
...
(`HostedZoneID`)|`**__Blank string__**`|Route 53-hosted zone ID of the domain name. If you don't provide an ACMSSLCertificateArn value, the Quick Start creates an ACM certificate for you using HostedZoneID in conjunction with DomainName.|ACM SSL certificate ARN
